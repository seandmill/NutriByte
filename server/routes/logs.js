...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const logs = await FoodLog.find({ userId: req.user._id }).sort({
      date: -1,
    });
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const logData = {
      ...
      logDate: logDate
        ? typeof logDate === "string" && logDate.length <= 10
...
router.get("/analytics", auth, async (req, res) => {
  try {
    ...
    if (brandOwners && brandOwners.length) {
      filter.brandOwner = {
        $in: Array.isArray(brandOwners) ? brandOwners : [brandOwners],
      };
    }
    ...
    if (brandNames && brandNames.length) {
      filter.brandName = {
        $in: Array.isArray(brandNames) ? brandNames : [brandNames],
      };
    }

    // Food category filtering
    if (foodCategories && foodCategories.length) {
      filter.brandedFoodCategory = {
        $in: Array.isArray(foodCategories) ? foodCategories : [foodCategories],
      };
    }
  ...
  try {
    ...
    const logsWithFixedNutrients = logs.map((log) => {
      ...
      if (!logObj.nutrients || Object.keys(logObj.nutrients).length === 0) {
        logObj.nutrients = {};
        if (logObj.calories)
          logObj.nutrients["1008"] = {
            id: "1008",
            value: logObj.calories,
            name: "Energy",
            unit: "kcal",
          };
        if (logObj.protein)
          logObj.nutrients["1003"] = {
            id: "1003",
            value: logObj.protein,
            name: "Protein",
            unit: "g",
          };
        if (logObj.fat)
          logObj.nutrients["1004"] = {
            id: "1004",
            value: logObj.fat,
            name: "Total Fat",
            unit: "g",
  ...
  try {
    ...
    logs.forEach((log) => {
      ...
      if (nutrients) {
        ...
        const nutrientsObj =
          nutrients instanceof Map ? Object.fromEntries(nutrients) : nutrients;
...
router.get("/export/csv", auth, async (req, res) => {
  try {
    ...
    const flattenedLogs = logs.map((log) => {
      ...
      const formattedDate = logObj.logDate
        ? new Date(logObj.logDate).toISOString().split("T")[0]
        : "";
      ...
      if (logObj.nutrients) {
        const nutrientsObj =
          logObj.nutrients instanceof Map
      ...
      return {
        ...
        Type: logType,
        Serving: `${logObj.quantity} ${
          logObj.servingDescription || logObj.servingType || "serving"
        }`,
    ...
    res
      .status(500)
      .json({ message: "Error generating CSV export", error: error.message });
  }
});
