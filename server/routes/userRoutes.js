...
router.get('/config', authMiddleware, async (req, res) => {
    try {
        ...
        const user = await User.findById(req.user._id).select('targetCalories macroPercentages overriddenNutrients');
        ...
        const configData = {
            targetCalories: user.targetCalories ?? DEFAULT_CONFIG_VALUES.targetCalories,
            macroPercentages: user.macroPercentages ? {
                 protein: user.macroPercentages.protein ?? DEFAULT_CONFIG_VALUES.macroPercentages.protein,
                 fat: user.macroPercentages.fat ?? DEFAULT_CONFIG_VALUES.macroPercentages.fat,
                 carbs: user.macroPercentages.carbs ?? DEFAULT_CONFIG_VALUES.macroPercentages.carbs,
             } : DEFAULT_CONFIG_VALUES.macroPercentages,
            // Convert Map to plain object, provide default if missing
            overriddenNutrients: Object.fromEntries(user.overriddenNutrients || new Map())
        };
...
router.put('/config', authMiddleware, async (req, res) => {
    ...
    if (macroPercentages) {
        ...
        if (Math.abs(protein + fat + carbs - 100) > 0.1) {
            return res.status(400).json({ message: 'Macro percentages must sum to 100%' });
        }
    }
    ...
    try {
        ...
        if (macroPercentages !== undefined) updatePayload.macroPercentages = macroPercentages;
        if (overriddenNutrients !== undefined) {
            // Convert incoming plain object back to Map for saving
            updatePayload.overriddenNutrients = new Map(Object.entries(overriddenNutrients));
        }
        ...
        // Prepare response object with potentially merged defaults (though findByIdAndUpdate should return full object)
         const responseData = {
            targetCalories: updatedUser.targetCalories ?? DEFAULT_CONFIG_VALUES.targetCalories,
            macroPercentages: updatedUser.macroPercentages ? {
                 protein: updatedUser.macroPercentages.protein ?? DEFAULT_CONFIG_VALUES.macroPercentages.protein,
                 fat: updatedUser.macroPercentages.fat ?? DEFAULT_CONFIG_VALUES.macroPercentages.fat,
                 carbs: updatedUser.macroPercentages.carbs ?? DEFAULT_CONFIG_VALUES.macroPercentages.carbs,
             } : DEFAULT_CONFIG_VALUES.macroPercentages,
            overriddenNutrients: Object.fromEntries(updatedUser.overriddenNutrients || new Map())
        };
