  ...
  try {
    ...
    if (response.data) {
      ...
      if (totalHits > 5000 && response.data.foods?.length > 0) {
        ...
        filteredFoods =
          response.data.foods?.filter((food) => food.score > 0.8) || [];
        ...
        if (filteredFoods.length === 0) {
          ...
          const sortedFoods = [...response.data.foods].sort(
            (a, b) => b.score - a.score
          );
          filteredFoods = sortedFoods.slice(
            0,
            Math.min(pageSize, sortedFoods.length)
          );
        }
      } else {
        // For normal searches, apply the standard filter
        filteredFoods =
          response.data.foods?.filter((food) => food.score > 0.5) || [];
      }
