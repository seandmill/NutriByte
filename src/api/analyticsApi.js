...
export const getMineralsSummary = async () => {
    try {
        ...
        console.error('Error fetching minerals data, calculating client-side:', error);
                ...
                if (Array.isArray(log.nutrients) && log.nutrients.length > 0) {
                    ...
                    log.nutrients.forEach(nutrient => {
                        if (nutrient.id && mineralIdMap[nutrient.id]) {
                            ...
                            mineralsData[mineral] += parseFloat(nutrient.value || nutrient.amount || 0);
                        }
                    });
                    ...
                    if (log.magnesium) mineralsData.magnesium += parseFloat(log.magnesium) || 0;
                    if (log.phosphorus) mineralsData.phosphorus += parseFloat(log.phosphorus) || 0;
                    if (log.potassium) mineralsData.potassium += parseFloat(log.potassium) || 0;
...
export const normalizeNutrientData = (logData) => {
    ...
    return logData.map(log => {
        ...
        if (log.calories !== undefined) log.nutrients['1008'] = { id: '1008', value: log.calories, name: 'Energy', unit: 'kcal' };
        if (log.protein !== undefined) log.nutrients['1003'] = { id: '1003', value: log.protein, name: 'Protein', unit: 'g' };
        if (log.fat !== undefined) log.nutrients['1004'] = { id: '1004', value: log.fat, name: 'Total Fat', unit: 'g' };
        if (log.carbs !== undefined) log.nutrients['1005'] = { id: '1005', value: log.carbs, name: 'Carbohydrates', unit: 'g' };
...
export const getCompleteNutrientData = async () => {
    try {
        ...
        console.log('Raw nutrients response:', response.data.length > 0 ? 'Success!' : 'Empty response');
        return response.data;
    } catch (err) {
        console.error('Failed to fetch raw nutrients, trying alternate approach:', err);

        // As a fallback, try direct MongoDB query via the server
        try {
            console.log('Attempting direct query...');
            const directQueryResponse = await axios.post(`${API_BASE}/logs/directquery`, {
            ...
            if (directQueryResponse.data && directQueryResponse.data.length > 0) {
                console.log('Direct query successful!', directQueryResponse.data.length, 'logs found');
                console.log('Sample log nutrients:', Object.keys(directQueryResponse.data[0].nutrients || {}).length, 'nutrient entries');
            ...
            if (regularResponse.data && regularResponse.data.length > 0) {
                console.log('Got', regularResponse.data.length, 'logs from regular endpoint');
