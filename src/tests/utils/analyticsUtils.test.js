import { calculateDailyAverage, getCalorieBreakdown, getTimeSeriesData, getFilterOptions } from '../../utils/analyticsUtils';
// Delete or comment out the jest import since it's already globally available
// import { jest } from '@jest/globals';

// Mock date-fns functions if needed, though direct Date objects might suffice
// jest.mock('date-fns', () => ({
...
//       // Basic format mock, extend if necessary
//       if (formatString === 'yyyy-MM-dd') {
//           return date.toISOString().split('T')[0];
//       }
//       return jest.requireActual('date-fns').format(date, formatString);
//   })
// }));
...
const sampleLogs = [
  // Log 1: Basic format, consumed
  {
    ...
    fat: 10,
    carbs: 30,
    nutrients: { // Mixed scalar and object values
      '1008': 300, // Calories
      '1003': { id: '1003', name: 'Protein', value: 20, unit: 'g' }, // Protein
      '1004': 10, // Fat
      '1005': { id: '1005', name: 'Carbohydrate, by difference', value: 30, unit: 'g' }, // Carbs
  ...
  // Log 2: Nutrient map only, consumed
  {
    ...
    nutrients: { // String keys, Object values
      ...
      '1005': { id: '1005', name: 'Carbohydrate, by difference', value: 40, unit: 'g' },
...
describe('analyticsUtils', () => {
  ...
  describe('calculateDailyAverage', () => {
    ...
    test('should calculate total calories correctly for consumed logs', () => {
      ...
      const totalCalories = calculateDailyAverage(sampleLogs, '1008', 'consumed', startDate, endDate, true);
     ...
     test('should calculate total protein correctly for avoided logs', () => {
      const totalProteinAvoided = calculateDailyAverage(sampleLogs, '1003', 'avoided', startDate, endDate, true);
       // Expected: 10 (log4)
      expect(totalProteinAvoided).toBe(10);
    });

    test('should return 0 if no logs match the type', () => {
      const result = calculateDailyAverage(sampleLogs, '1008', 'nonexistent', startDate, endDate, true);
      expect(result).toBe(0);
    });

    test('should return 0 for empty logs array', () => {
      const result = calculateDailyAverage(emptyLogs, '1008', 'consumed', startDate, endDate, true);
      expect(result).toBe(0);
    });

    // TODO: Add more tests:
    // - Test different nutrient IDs (iron, sodium etc.)
    // - Test calculation without start/end dates (should default to daysCount = 1 if showTotal=false?) - Check logic
    // - Test edge case date ranges (single day)
  });
  ...
  describe('getCalorieBreakdown', () => {
    ...
    test('should calculate calorie breakdown percentages correctly for consumed logs', () => {
      ...
      // Carb % = Math.round((520 / 1362) * 100) = Math.round(38.18) = 38
       expect(breakdown).toEqual({ proteinPercent: 23, fatPercent: 38, carbsPercent: 38 });
     });

    test('should return all zeros if no matching logs', () => {
      const breakdown = getCalorieBreakdown(sampleLogs, 'nonexistent');
      expect(breakdown).toEqual({ proteinPercent: 0, fatPercent: 0, carbsPercent: 0 });
    });
     ...
     test('should return all zeros for empty logs array', () => {
      ...
      test('should return all zeros if total calories from macros are zero', () => {
       const zeroMacroLogs = [{ logType: 'consumed', date: new Date(), nutrients: { '1003': 0, '1004': 0, '1005': 0 }}];
  ...
  describe('getTimeSeriesData', () => {
    ...
    test('should return empty array if no matching logs', () => {
     ...
     // TODO: Add tests for other nutrient IDs and log types
  });

  describe('getFilterOptions', () => {
    // Test cases for getFilterOptions will go here
     test('should extract unique and sorted filter options from logs', () => {
       const options = getFilterOptions(sampleLogs);

       expect(options.brandOwners).toEqual(['Good Food Co.', 'Healthy Eats Inc.', 'Meal Prep Pro', 'Tasty Treats']);
       expect(options.brandNames).toEqual(['Good Brand', 'Healthy Choice', 'Super Snack', 'Sugar Bomb', 'Chicken & Veg'].sort());
       expect(options.foodCategories).toEqual(['Desserts', 'Meals', 'Snacks']);
       expect(options.ingredients).toEqual([ // Needs careful checking based on parsing logic
         'Chicken', 'Rice', 'Broccoli', 'Sauce', // From log 1 (split by comma)
         'Oats', 'Honey', 'Nuts', // From log 2 (parsedIngredients)
         'Flour', 'Sugar', 'Eggs', // From log 3 (split by semicolon)
         // Ingredients from avoided/prepped logs (4, 5) are not extracted by current logic
       ].sort());
     });
     ...
     test('should handle logs missing ingredients or brand info gracefully', () => {
       const partialLogs = [
         { _id: 'log1', logType: 'consumed', date: new Date(), brandOwner: 'Owner A'},
         { _id: 'log2', logType: 'consumed', date: new Date(), brandName: 'Brand B'},
         { _id: 'log3', logType: 'consumed', date: new Date(), brandedFoodCategory: 'Category C'},
         { _id: 'log4', logType: 'consumed', date: new Date(), ingredients: 'Ingredient D'},
         { _id: 'log5', logType: 'consumed', date: new Date(), parsedIngredients: ['Ingredient E']},
       ];
        ...
        expect(options.ingredients).toEqual(['Ingredient D', 'Ingredient E']); // Assuming simple split for Ingredient D
     });
   });
