...
const NutrientCard = ({ title, value, unit }) => {
  ...
  const dvPercent = NUTRIENT_DV[nutrientKey]
    ? Math.min(100, Math.round((value / NUTRIENT_DV[nutrientKey]) * 100))
    : 0;
  ...
  return (
    <div data-testid="nutrient-card" aria-label={`${title}: ${value}${unit}`}>
      ...
      {dvPercent > 0 && <div data-testid="nutrient-dv">{dvPercent}% DV</div>}
    </div>
  );
};
...
describe("NutrientCard Component", () => {
  ...
  it("displays the formatted value with unit", () => {
    render(<NutrientCard title="Protein" value={25} unit="g" />);
  ...
  it("handles zero values correctly", () => {
    render(<NutrientCard title="Calories" value={0} unit="kcal" />);
