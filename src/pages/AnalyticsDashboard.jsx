...
import { getMineralsSummary, normalizeNutrientData, getCompleteNutrientData } from '../api/analyticsApi';
import { 
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'baseline' }}>
        <Typography variant="h3" component="div" fontWeight="bold" sx={{ flexGrow: 1 }}>
          {total}
          <Typography component="span" variant="subtitle1" fontWeight="normal" sx={{ ml: 0.5 }}>
            {unit}
          </Typography>
        </Typography>
      ...
      <Box sx={{ mt: 1 }}>
        <Typography variant="body2" component="div" fontStyle="italic" color="text.secondary">
...
const TrendLineChart = ({ title, data, labels, color, borderColor, fillColor }) => {
  const chartData = {
    labels,
    datasets: [
      {
...
const CompositionChart = ({ title, data, labels, colors }) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', height: 250 }}>
      ...
      <AccordionDetails>
        <Grid container spacing={2}>
          {nutrients.map(nutrient => {
            ...
            let average = 0;
            let timeSeriesData = getTimeSeriesData(logs, nutrient.id.toString(), logType);
            ...
            if (categoryKey === 'minerals' && directMineralData) {
              ...
              if (directMineralData[mineralName] !== undefined) {
                ...
                average = calculateDailyAverage(logs, nutrient.id.toString(), logType, startDate, endDate, true);
              }
            } else {
              ...
              <Grid item xs={12} md={6} key={nutrient.id}>
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, mb: 2 }}>
                    ...
                    <Typography component="span" variant="subtitle2" sx={{ ml: 0.5, minWidth: '55px' }}>
                      {nutrient.unit}/day
                    </Typography>
                    {nutrient.dv && (
                      <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center' }}>
                        <Typography variant="body2" color="text.secondary" sx={{ mr: 1, minWidth: '80px', textAlign: 'right' }}>
                        ...
                        >
                          <Box 
                            sx={{ 
                              width: `${Math.min(100, Math.round((average / nutrient.dv) * 100))}%`, 
                              height: '100%',
                  ...
                  {timeSeriesData.length > 1 && (
                    <Box sx={{ height: 150 }}>
                      <Line 
                        data={{
                          labels: timeSeriesData.map(d => format(parseISO(d.date), 'MMM d')),
                          datasets: [{
                            label: nutrient.name,
          ...
          }).filter(Boolean)} {/* Filter out null items (zero values) */}
        </Grid>
      </AccordionDetails>
    </Accordion>
  );
...
const AnalyticsDashboard = () => {
  ...
  }, [logs, startDate, endDate, logType, selectedBrandOwners, selectedBrandNames, selectedFoodCategories, selectedIngredients]);
    ...
    try {
      ...
      try {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          ...
          console.log('✅ getCompleteNutrientData succeeded with', data.length, 'logs');
        } else {
          console.warn('⚠️ getCompleteNutrientData returned empty or invalid data');
        }
      } catch (err) {
      ...
      if (response.length === 0) {
        try {
          ...
          if (data && Array.isArray(data) && data.length > 0) {
            ...
            console.log('✅ getFilteredFoodLogs succeeded with', data.length, 'logs');
          } else {
            console.warn('⚠️ getFilteredFoodLogs returned empty or invalid data');
          }
        } catch (err) {
      ...
      if (response.length === 0) {
        try {
          ...
          if (data && Array.isArray(data) && data.length > 0) {
            response = data;
            source = 'direct API';
            console.log('✅ Direct API call succeeded with', data.length, 'logs');
          } else {
      ...
      console.log(`Successfully retrieved ${response.length} logs from ${source}`);
      ...
      if (normalizedLogs.length > 0) {
        ...
        console.log('First log data:', JSON.stringify(normalizedLogs[0], null, 2));
        
        // Specifically check nutrients 
        if (normalizedLogs[0].nutrients) {
          console.log('Nutrients keys:', Object.keys(normalizedLogs[0].nutrients));
          
          // Check for calcium specifically
          if (normalizedLogs[0].nutrients['1087']) {
            console.log('Calcium data in first log:', normalizedLogs[0].nutrients['1087']);
          } else {
        ...
        if (calciumLogs.length > 0) {
          console.log('Calcium data example:', calciumLogs[0].nutrients['1087']);
        }
      } else {
  ...
  const applyFilters = () => {
    ...
    if (selectedBrandOwners.length > 0) {
      filtered = filtered.filter(log => 
        log.brandOwner && selectedBrandOwners.includes(log.brandOwner)
      );
    }
    ...
    if (selectedBrandNames.length > 0) {
      filtered = filtered.filter(log => 
        log.brandName && selectedBrandNames.includes(log.brandName)
      );
    }
    ...
    if (selectedFoodCategories.length > 0) {
      filtered = filtered.filter(log => 
        log.brandedFoodCategory && selectedFoodCategories.includes(log.brandedFoodCategory)
      );
    }
  ...
  const handleLogTypeChange = (event, newLogType) => {
    if (newLogType !== null) {
      ...
      const isPreppedToggle = 
        (newLogType === 'prepped' && (logType === 'consumed' || logType === 'avoided')) ||
        (logType === 'prepped' && (newLogType === 'consumed' || newLogType === 'avoided'));
      ...
      if (isPreppedToggle) {
        if (newLogType === 'prepped') {
          ...
          const relevantLogs = logs.filter(log => 
            log.logType === 'consumed' || log.logType === 'avoided'
          );
          
          if (relevantLogs.length > 0) {
            const dates = relevantLogs.map(log => new Date(log.logDate || log.date));
  ...
  // Calculate analytics
  const calorieAvg = calculateDailyAverage(filteredLogs, '1008', logType, startDate, endDate, true);
  const proteinAvg = calculateDailyAverage(filteredLogs, '1003', logType, startDate, endDate, true);
  const fatAvg = calculateDailyAverage(filteredLogs, '1004', logType, startDate, endDate, true);
  const carbsAvg = calculateDailyAverage(filteredLogs, '1005', logType, startDate, endDate, true);
  const fiberAvg = calculateDailyAverage(filteredLogs, '1079', logType, startDate, endDate, true);
  ...
  const timeLabels = caloriesTimeSeries.map(d => format(parseISO(d.date), 'MMM d'));
        ...
        <Box sx={{ mt: 2, mb: 4 }}>
          ...
          {loading ? (
            ...
            <>
                ...
                <Grid container spacing={2} alignItems="center">
                      ...
                      >
                        {filterOptions.brandOwners.map((brand) => (
                          <MenuItem key={brand} value={brand}>
                            <Checkbox checked={selectedBrandOwners.indexOf(brand) > -1} />
                    ...
                    <FormControl size="small" fullWidth>
                      ...
                      >
                        {filterOptions.brandNames.map((brand) => (
                          <MenuItem key={brand} value={brand}>
                            <Checkbox checked={selectedBrandNames.indexOf(brand) > -1} />
                            <ListItemText primary={brand} />
                          </MenuItem>
                        ))}
                      </Select>
                  ...
                  <Grid item xs={12} sm={6} md={3} lg={1}>
                    ...
                    <FormControl size="small" fullWidth>
                      <Select
                        ...
                        onChange={(e) => setSelectedFoodCategories(e.target.value)}
                        input={<OutlinedInput />}
                        renderValue={(selected) => selected.length > 0 ? `${selected.length} selected` : "None"}
                        displayEmpty
                      >
                        {filterOptions.foodCategories.map((category) => (
                          <MenuItem key={category} value={category}>
                            <Checkbox checked={selectedFoodCategories.indexOf(category) > -1} />
                  ...
                  <Grid item xs={12}>
                    <Typography variant="body2" color="text.secondary">
                      ...
                      {startDate && endDate ? ` over ${differenceInDays(endDate, startDate) + 1} days` : ''}
                    </Typography>
                  </Grid>
                </Grid>
              </Paper>
              ...
              {/* Charts Row */}
              <Paper elevation={2} sx={{ p: 2, mb: 3, borderRadius: 2 }}>
                <Grid container spacing={4} sx={{ display: 'flex', justifyContent: 'stretch' }}>
                  {/* Calories Line Chart */}
                  <Grid item xs={12} md={4} sx={{ flexGrow: 1, width: '33%' }}>
                    <Paper elevation={1} sx={{ p: 2, height: '100%', borderRadius: 2, border: '1px solid rgba(0,0,0,0.1)' }}>
                      <Typography variant="h6" color="text.secondary" fontWeight="medium" sx={{ mb: 2 }}>
                        Calories Over Time
                      </Typography>
                      <Box sx={{ height: '250px' }}>
              ...
              <Paper elevation={2} sx={{ p: 2, borderRadius: 2 }}>
                {nutrientGroups.map(group => (
                  <NutrientGroup
                    ...
                    startDate={startDate}
                    endDate={endDate}
                    directMineralData={group.key === 'minerals' ? mineralData : null}
                  />
                ))}
              </Paper>
            </>
