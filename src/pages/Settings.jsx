...
const Settings = () => {
    ...
    const fetchConfig = async () => {
      ...
      try {
        ...
        setConfig({
          ...
          macroPercentages: fetchedConfig.macroPercentages || {
            protein: 20,
            fat: 30,
            carbs: 50,
          },
  ...
  const calculatedDefaultTargets = useMemo(() => {
    ...
    Object.entries(NUTRIENT_METADATA).forEach(([id, meta]) => {
      if (meta.dv && meta.category !== "macronutrients") {
        // Only scale non-macros with a DV
        targets[id] = {
          value: (meta.dv * calorieRatio).toFixed(
            meta.unit === "mg" || meta.unit === "g" ? 1 : 0
  ...
  useEffect(() => {
    ...
    if (Math.abs(total - 100) > 0.1) {
      // Use tolerance
      setMacroError(
        `Macro percentages must sum to 100% (Currently: ${total.toFixed(1)}%)`
      );
    } else {
      setMacroError("");
    }
        ...
        <Paper sx={{ p: 3 }}>
          ...
          <Box mb={3}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Set specific targets for individual nutrients below. Any value
              entered here will override the default calculated target based on
              your calorie goal.
            </Typography>
                  ...
                  <AccordionDetails>
                    <List dense disablePadding>
                      {Object.entries(NUTRIENT_METADATA)
                        ...
                        .map(([nutrientId, meta]) => {
                          const override =
                            config.overriddenNutrients[nutrientId];
                          const defaultValue = calculatedDefaultTargets[
                            nutrientId
                          ] || { value: "N/A", unit: meta.unit };
                          ...
                          return (
                            ...
                            >
                              ...
                              <TextField
                                ...
                                onChange={(e) =>
                                  handleOverrideChange(e, nutrientId, meta.unit)
                                }
                                InputProps={{
                                  endAdornment: (
                              ...
                              <Tooltip title="Reset to Default">
                                ...
                                <span>
                                  ...
                                  <IconButton
                                    onClick={() =>
                                      handleResetOverride(nutrientId)
                                    }
