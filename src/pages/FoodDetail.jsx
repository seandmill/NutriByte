...
const AllergenWarningBanner = ({ ingredients }) => {
  ...
  const bigNineAllergens = [
    ...
    { 
      name: 'Eggs', 
      severity: 'high',
      keywords: ['egg', 'albumin', 'ovalbumin', 'lysozyme', 'globulin', 'mayonnaise', 'meringue']
    },
    { 
      name: 'Fish', 
      severity: 'high',
      keywords: ['fish', 'cod', 'salmon', 'tuna', 'anchovy', 'bass', 'trout', 'halibut', 'mahi', 'tilapia']
    },
    { 
      name: 'Crustacean Shellfish', 
      severity: 'high',
      keywords: ['shellfish', 'crab', 'lobster', 'shrimp', 'prawn', 'crayfish', 'langoustine', 'krill']
    },
    { 
      name: 'Tree Nuts', 
      severity: 'high',
      keywords: ['almond', 'hazelnut', 'walnut', 'cashew', 'pecan', 'brazil nut', 'pistachio', 'macadamia', 'pine nut', 'chestnut']
    },
    { 
      name: 'Peanuts', 
      severity: 'high',
    ...
    { 
      name: 'Wheat', 
      severity: 'high',
      keywords: ['wheat', 'flour', 'bread', 'pasta', 'semolina', 'couscous', 'bulgur', 'farina', 'seitan']
    },
    { 
      name: 'Soybeans', 
      severity: 'high',
      keywords: ['soy', 'soya', 'edamame', 'tofu', 'tempeh', 'miso', 'natto', 'tamari']
    },
    { 
      name: 'Sesame', 
      severity: 'high',
  ...
  const detectedAllergens = bigNineAllergens.filter(allergen => 
    allergen.keywords.some(keyword => ingredientsLower.includes(keyword.toLowerCase()))
  );
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary">
        Always verify allergen information directly with the manufacturer as ingredients and formulations may change.
      </Typography>
    </Paper>
  );
};
...
const FoodDetail = () => {
  ...
  const [servingUnitInfo, setServingUnitInfo] = useState({ size: 100, unit: 'g', household: '' });
  ...
  const [userMacroPercentages, setUserMacroPercentages] = useState({ protein: 20, fat: 30, carbs: 50 }); // Default values
  ...
  const loadFoodDetails = async () => {
    ...
    try {
      ...
      setServingUnitInfo({ size: initialServingSize, unit: initialServingUnit, household: initialHouseholdServing });
  ...
  const handleLogFood = async () => {
    ...
    Object.values(baseNutrientData.nutrients || {}).flat().forEach(n => {
    ...
    if (!nutrientIdMap['Energy'] && !nutrientIdMap['Calories']) {
      ...
      const energyNutrient = baseNutrientData.nutrients?.macronutrients?.find(n => n.id === 1008);
      if (energyNutrient) {
        const scaledValue = ((Number(energyNutrient.value) || 0) * scaleFactor).toFixed(2);
    ...
    if (mealPrepActive) {
      ...
      try {
        if (prepMode === 'days') {
           if (prepDays > 0) {
              targetDates = eachDayOfInterval({ start: today, end: addDays(today, prepDays - 1) });
           }
          ...
          if (prepWeeks > 0) {
            ...
            const intervalDays = eachDayOfInterval({ start: today, end: endDate });
        ...
        console.log(`Preparing to log ${targetDates.length} meal prep entries...`);
        ...
        setTimeout(() => navigate(`/dashboard?logDate=${encodeURIComponent(dateForRedirect)}`), 1500);

      } catch(prepError) {
           console.error('Meal prep logging failed:', prepError);
           setError(`Meal prep failed: ${prepError.message || 'Please check configuration.'}`);
      }
      // ----------------------
    } else {
      // --- Single Log Logic --- 
      try {
        ...
        const logDate = logDateParam ? createUTCNoonDate(logDateParam) : createUTCNoonDate(new Date());
  ...
  if (error && !food) {
    return (
      <Layout>
        <Container>
           <Button startIcon={<ArrowBackIcon />} onClick={handleBackToSearch} sx={{ mb: 2 }}> Back </Button>
  ...
  const displayScaleFactor = isZeroServing ? 0 : (Number(currentGrams) || 0) / 100;

  // Calculate DV percent with user settings for macros
  const calculateDVPercent = (nutrientValue, nutrientUnit, dvAmount, dvUnit, nutrientId, nutrientName) => {
    // Handle Energy/Calories (nutrient ID 1008)
    if (nutrientId === 1008) {
      return Math.min(100, Math.round((nutrientValue / userTargetCalories) * 100));
    }
    
    // Special handling for macronutrients based on user's calories and macro percentages
    if (nutrientName && ['Protein', 'Total Fat', 'Carbohydrates'].includes(nutrientName)) {
      let effectiveDV;
      
      if (nutrientName === 'Protein') {
        // Protein: calories * protein% / 4 calories per gram
        effectiveDV = (userTargetCalories * (userMacroPercentages.protein / 100)) / CALORIE_FACTORS.Protein;
      } else if (nutrientName === 'Total Fat') {
        // Fat: calories * fat% / 9 calories per gram
        effectiveDV = (userTargetCalories * (userMacroPercentages.fat / 100)) / CALORIE_FACTORS['Total Fat'];
      } else if (nutrientName === 'Carbohydrates') {
        // Carbs: calories * carbs% / 4 calories per gram
        effectiveDV = (userTargetCalories * (userMacroPercentages.carbs / 100)) / CALORIE_FACTORS.Carbohydrates;
      }
    ...
    else if (nutrientUnit === 'µg' && effectiveDVUnit === 'mg') valueInDvUnit /= 1000;
    else if (nutrientUnit === 'µg' && effectiveDVUnit === 'g') valueInDvUnit /= 1000000;
  ...
  return (
    <Layout>
      <Container maxWidth="lg">
        ...
        {food.ingredients && <AllergenWarningBanner ingredients={food.ingredients} />}

        {isLogging && <Alert severity="info" sx={{ mb: 2 }}>Processing log entry...</Alert>}
        {error && (
        ...
        {logSuccess && typeof logSuccess === 'boolean' && logSuccess === true && (
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 2, position: 'sticky', top: '80px' }}>
              ...
              <TextField
                type="number"
                label={servingUnitInfo.household ? servingUnitInfo.household : servingUnitInfo.unit}
                value={servingAmount}
                onChange={handleServingAmountChange}
                fullWidth
                sx={{ mb: 2 }}
                 inputProps={{ step: "0.1" }}
                 helperText={servingUnitInfo.household ? `Equals ${(Number(currentGrams) || 0).toFixed(1)} ${servingUnitInfo.unit}` : 'Enter amount in grams'}
              />
              
              <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 1.5, mb: 2, backgroundColor: mealPrepActive ? 'action.hover' : 'transparent', opacity: isLogging ? 0.5 : 1 }}>
                 <FormControlLabel
                    control={<Switch checked={mealPrepActive} onChange={(e) => setMealPrepActive(e.target.checked)} disabled={isLogging} />}
                 ...
                 {mealPrepActive && (
                     <Box pl={1}>
                         <RadioGroup
                            ...
                            value={prepMode}
                            onChange={(e) => !isLogging && setPrepMode(e.target.value)}
                            name="meal-prep-mode-group"
                         >
                            <FormControlLabel value="days" control={<Radio size="small" disabled={isLogging}/>} label={<Box sx={{ display: 'flex', alignItems: 'center' }}>Next&nbsp;<TextField value={prepDays} disabled={isLogging} onChange={(e) => !isLogging && setPrepDays(Math.max(1, Number(e.target.value) || 1))} type="number" size="small" sx={{ width: '60px', mx: 0.5 }} inputProps={{min: 1}} onClick={(e)=> e.stopPropagation()} onFocus={()=>setPrepMode('days')} />&nbsp;Days</Box>} />
                            
                            <FormControlLabel value="week" control={<Radio size="small" disabled={isLogging}/>} label="This Week (Rest of Week)" />
                            
                            <FormControlLabel value="custom" control={<Radio size="small" disabled={isLogging}/>} label="Custom Schedule" />
                            {prepMode === 'custom' && (
                               <Box sx={{ pl: 3, mt: 0.5, mb: 1 }}>
                                  <FormGroup row sx={{ mb: 1 }}>
                                     {Object.keys(prepSelectedDays).map(day => (
                                         <FormControlLabel key={day} control={<Checkbox size="small" checked={prepSelectedDays[day]} onChange={handlePrepDayChange} name={day} disabled={isLogging} />} label={day.charAt(0).toUpperCase() + day.slice(1,3)} sx={{mr:0.5}}/>
                                     ))}
                                  </FormGroup>
                                   <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                     For the Next&nbsp;<TextField value={prepWeeks} disabled={isLogging} onChange={(e) => !isLogging && setPrepWeeks(Math.max(1, Number(e.target.value) || 1))} type="number" size="small" sx={{ width: '60px', mx: 0.5 }} inputProps={{min: 1}} onClick={(e)=> e.stopPropagation()}/>&nbsp;Weeks
                                    </Box>
                               </Box>
                            )}
                         </RadioGroup>
              ...
              <Button
                ...
                disabled={!servingAmount || Number(servingAmount) <= 0 || isLogging || (mealPrepActive && ((prepMode === 'days' && prepDays <=0) || (prepMode === 'custom' && prepWeeks <=0))) }
                sx={{ mb: 1 }}
              >
                {isLogging ? 'Logging...' : (mealPrepActive ? 'Log Meal Prep' : 'Log Food')}
              </Button>
              ...
              <Button
                ...
                color="secondary"
                fullWidth
                disabled={isLogging || !servingAmount || Number(servingAmount) <= 0}
          ...
          <Grid item xs={12} md={8}>
            ...
            {Object.entries(NUTRIENT_CATEGORIES).map(([categoryKey, categoryName]) => {
              const baseNutrientsInCategory = baseNutrientData.nutrients?.[categoryKey]
                 ?.filter(n => n.value > 0);

              if (!baseNutrientsInCategory || baseNutrientsInCategory.length === 0) {
                return null;
              }
              ...
              const nutrientCards = baseNutrientsInCategory.map(n => {
                    ...
                    const dvPercent = calculateDVPercent(scaledValue, n.unit, metadata.dv, metadata.unit, n.id, n.name);
                    const calorieContribution = (metadata.category === 'macronutrients' && CALORIE_FACTORS[n.name])
                    ...
                    const isUsingCustomTarget = 
                      ...
                      (n.name === 'Protein' || n.name === 'Total Fat' || n.name === 'Carbohydrates') ||
                    ...
                    <Grid container spacing={1}>
                      {nutrientCards.map((nutrient) => (
                        <Grid item xs={6} sm={4} md={3} key={nutrient.id}>
                           <Card variant="outlined" sx={{ height: '100%', minHeight: '150px', width: '100%' }}>
                             <CardContent sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', p: 1.5 }}>
                               <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                ...
                                <Typography variant="h6" component="div" align="center" sx={{ mb: 1, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                   {formatNutrientValue(nutrient.scaledValue, nutrient.unit)}
                                  {nutrient.calorieContribution !== null && !isZeroServing && (
                                       <Typography variant="caption" display="block" color="text.secondary">
                                           (~{nutrient.calorieContribution.toFixed(0)} kcal)
                                       </Typography>
                                  )}
                                </Typography>
                                ...
                                {!isZeroServing && nutrient.dvPercent > 0 ? (
                                   <Box sx={{ width: '100%', height: '24px' }}>
                                      <LinearProgress variant="determinate" value={nutrient.dvPercent} sx={{ height: 8, borderRadius: 4, mb: 0.5 }}/>
                                      <Typography variant="caption" display="block" align="right">
                                          {nutrient.dvPercent}% DV
                                          {nutrient.isUsingCustomTarget ? (
                                            <Tooltip title="Using your custom nutrient target">
                                              <span style={{ marginLeft: 4, color: 'primary.main' }}>✓</span>
                                            </Tooltip>
                                          ) : (
             ...
             {Object.values(baseNutrientData.nutrients || {}).flat().length === 0 && !loading && (
                 <Typography color="textSecondary" sx={{mt: 2}}>No nutrient data available for this item.</Typography>
             )}
          </Grid>
        </Grid>
      </Container>
